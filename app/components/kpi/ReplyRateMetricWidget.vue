...
import type { AiReplyRateMetric, MetricDescriptor } from '~/types/kpi'

const props = defineProps<{
  descriptor: MetricDescriptor
  data: AiReplyRateMetric | null
  loading?: boolean
  error?: string | null
}>()
...
const summaryItems = computed(() => {
  ...
  return [
    { label: 'AI Reply Rate', value: formatPercent(props.data.summary.aiReplyRate) },
    { label: 'AI Replies', value: String(props.data.summary.aiReplies) },
    { label: 'Human Replies', value: String(props.data.summary.humanReplies) },
    ...
    <div class="px-6 py-5 border-b border-black/8">
      <div class="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        ...
        >
          ...
          <p class="text-2xl font-semibold mt-2">{{ formatPercent(data.summary.aiReplyRate) }}</p>
    ...
    <div class="px-6 py-6 space-y-6">
      ...
      <template v-else-if="data">
        ...
        <div class="rounded-2xl border border-black/8 bg-white/60 px-4 py-4">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Successful Outgoing Replies</p>
          <p class="text-2xl font-semibold text-zinc-950 mt-2">{{ data.summary.totalSuccessfulOutgoingReplies }}</p>
          <p class="text-sm text-zinc-500 mt-1">AI-generated successful replies vs all successful outgoing replies.</p>
        </div>
